...
console.log(orders)
  ...
  const onOrderCompleteHandler =async(id:string)=>{
    
    const dbRef = doc(db, "orders",id);
    const response = await updateDoc(dbRef,{status:'delivered'})
  ...
  return (
    <>
      <main className="ml-[17rem]">
        <h1 className="text-3xl font-bold">Orders To Deliever </h1>
          ...
          if(item.status === 'packing'){
            return (
              ...
              >
                ...
                <button className="px-6 py-3 bg-indigo-600 text-white rounded-md " onClick={()=>onOrderCompleteHandler(item.id)} >Order Delivered</button>
                </div>
